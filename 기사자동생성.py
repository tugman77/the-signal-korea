...
# 하루 발행 건수 (2026-09-22 대표님 지시로 5→3). 바꾸려면 이 값만 고친다.
...
"""
    ...
    # ── 발행 건수 (2026-09-22: 5건 → 3건) ──────────────────────────
    # 5건일 때 비슷한 주제가 반복된다는 지적이 있었다. 수집 풀이 30건/일이고
    # 오피니언 매체라 하루 5편은 '새 사건'보다 '각도 늘리기'가 되기 쉽다.
    # 5건은 배치 2개(3+2)로 나눠야 했다 — 한 호출로 뽑으면 출력이 32,000토큰
    # 상한에 걸려 JSON이 잘린 채 rc=1로 끝났기 때문이다(07-25~08-04 발행 실패 원인).
    # 3건은 한 배치로 상한 아래이므로 배치 분할이 필요 없다.
    # 카테고리는 4종을 매일 다 채울 수 없으니 공급망전쟁(간판)만 고정하고
    # 나머지 2건은 남은 3종에서 오늘 사안에 맞게 고르게 한다 —
    # 고정 쿼터로 묶으면 사건이 없는 카테고리를 억지로 채우게 된다.
    batches = [
        {
            "count": ARTICLE_COUNT,
            "featured": True,
            "quota": ("[이번 배치 카테고리 — 반드시 준수]\n"
                      "- 공급망전쟁: 1~2개 — 갈륨·탄탈럼·희토류·리튬 등 소재 중심\n"
                      "- 나머지는 기술패권 / 산업전략 / 글로벌분석 중에서 고른다.\n"
                      "  3건이 같은 카테고리에 몰리면 안 된다(최소 2개 카테고리).\n"
                      "- 하루 3건뿐이다. 건수를 채우려고 한 사건을 여러 각도로\n"
                      "  늘려 쓰지 말고, 오늘 가장 중요한 사안만 골라 깊게 쓴다."),
        },
    ]
